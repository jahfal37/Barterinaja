...
<body>
    ...
    <div class="container py-5">
        <!-- Judul Pencarian -->
        @if ($query && !$city)
            <h1 class="text-2xl font-semibold mb-4">Hasil Pencarian untuk Produk: "{{ $query }}"</h1>
        @elseif ($city && !$query)
            <h1 class="text-2xl font-semibold mb-4">Hasil Pencarian untuk Kota: "{{ $city }}"</h1>
        @elseif ($query && $city)
            <h1 class="text-2xl font-semibold mb-4">Hasil Pencarian untuk Produk: "{{ $query }}" di Kota: "{{ $city }}"</h1>
        @else
            <h1 class="text-2xl font-semibold mb-4">Hasil Pencarian</h1>
        @endif
                        ...
                        <a href="{{ route('item.show', $item->id) }}" class="block text-decoration-none text-gray-800">
                            <div class="relative">
                                @if ($item->screenshots->isNotEmpty())
                                    <img src="{{ asset('storage/' . $item->screenshots->first()->path) }}" alt="{{ $item->product_name }}"
